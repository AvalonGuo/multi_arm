...
class PyBulletRobot(ABC):
    ...
    def set_action(self, action: np.ndarray) -> None:
        ...
        """
    @abstractmethod
    def pid_action(self,goal:np.ndarray) -> None:
        """
        """
    @abstractmethod
    ...
    def set_base_pose(self,body: str, position: np.ndarray, orientation: np.ndarray) -> None:
        self.sim.set_base_pose(body=body,position=position,orientation=orientation)
...
class RobotTaskEnv(gym.Env):
    ...
    def _get_obs(self) -> Dict[str, np.ndarray]:
        ...
        finger_width = np.array([self.robot.get_fingers_width()])
        achieved_goal = np.concatenate([achieved_goal,finger_width])
        return {
