...
class Panda(PyBulletRobot):
    ...
    def close_gripper(self)->None:
        ...
        gripper_forces = [170.0,170.0]
        target_angles = np.array([target_fingers_width / 2, target_fingers_width / 2])
        self.sim.control_joints(self.body_name,gripper_indices,target_angles,gripper_forces)




    def pid_action(self,action:np.ndarray)->None:
        action = action.copy()
        target_arm_angles = self.pid_ee_displacement_to_target_arm_angles(action[:3])
        target_fingers_width = action[-1]
    ...
    def pid_ee_displacement_to_target_arm_angles(self, ee_displacement: np.ndarray) -> np.ndarray:
